...
export default function Home() {
  ...
  // ACCESSIBILITY: Disable hover zooms and transitions if reduced motion is true
  const animScale = reducedMotion ? "" : "group-hover:scale-110 transition-transform";
  const animTransition = reducedMotion ? "" : "transition-all";
    ...
    <div className="min-h-[80vh] flex flex-col justify-center max-w-5xl mx-auto p-6 space-y-12 my-8">
      ...
      <div className="max-w-2xl mx-auto w-full text-center space-y-6">
        ...
        <div className="relative w-full mt-8 shadow-sm">
          ...
          {/* ACCESSIBILITY: Explicit screen reader label tied to input ID */}
          <label htmlFor="campus-search" className="sr-only">Search the campus database</label>
          <input 
            id="campus-search"
            type="text"
            value={query}
            ...
            className="w-full pl-12 pr-4 py-4 rounded-full border-2 border-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 text-lg bg-white text-slate-900"
          />
        </div>
      </div>

      {/* Conditional Rendering: Show Results OR Buttons */}
      {/* ACCESSIBILITY: aria-live reads out search changes to screen readers automatically */}
      <div aria-live="polite" className="w-full">
        {query.trim() !== "" ? (
            ...
            {searchResults.length > 0 ? (
              searchResults.map((res, i) => (
                <Link 
                  ...
                  className="block focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 rounded-xl"
                >
                  <div className={`p-4 bg-white border border-slate-200 rounded-xl hover:border-sky-500 flex justify-between items-center group mb-3 shadow-sm ${animTransition}`}>
                    <div>
                    ...
                    <ArrowRight aria-hidden="true" className={`text-slate-300 group-hover:text-sky-500 ${animTransition}`} />
                  </div>
                </Link>
              ))
            ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <Link 
              href="/timetable" 
              className={`flex flex-col items-center justify-center p-8 bg-white border-2 border-slate-100 rounded-2xl shadow-sm hover:border-sky-400 group focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${animTransition}`}
            >
              <Calendar size={48} aria-hidden="true" className={`text-sky-600 mb-4 ${animScale}`} />
            ...
            <Link 
              ...
              className={`flex flex-col items-center justify-center p-8 bg-white border-2 border-slate-100 rounded-2xl shadow-sm hover:border-sky-400 group focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${animTransition}`}
            >
              <Utensils size={48} aria-hidden="true" className={`text-sky-600 mb-4 ${animScale}`} />
            ...
            <Link 
              ...
              className={`flex flex-col items-center justify-center p-8 bg-white border-2 border-slate-100 rounded-2xl shadow-sm hover:border-sky-400 group focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${animTransition}`}
            >
              <Map size={48} aria-hidden="true" className={`text-sky-600 mb-4 ${animScale}`} />
            ...
            <Link 
              ...
              className={`flex flex-col items-center justify-center p-8 bg-white border-2 border-slate-100 rounded-2xl shadow-sm hover:border-sky-400 group focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${animTransition}`}
            >
              <HelpCircle size={48} aria-hidden="true" className={`text-sky-600 mb-4 ${animScale}`} />
            ...
            <Link 
              ...
              className={`flex flex-col items-center justify-center p-8 bg-white border-2 border-slate-100 rounded-2xl shadow-sm hover:border-sky-400 group focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${animTransition}`}
            >
              <FileQuestion size={48} aria-hidden="true" className={`text-sky-600 mb-4 ${animScale}`} />
