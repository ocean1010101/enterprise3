...
export default function Canteen() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 my-8 space-y-8">
      {/* Banner now uses standard styling so it matches the rest of the app in Dark Mode */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {/* Daily Specials */}
        <div className="md:col-span-1 bg-sky-50 p-6 rounded-xl shadow-md border-2 border-sky-200">
            ...
            {specialMenu.map((item, i) => (
              <li key={i} className="bg-white p-4 rounded shadow-sm border border-sky-100">
                <div className="flex justify-between items-start font-bold text-slate-800">
                  <span>{item.name}</span>
                  <span className="text-sky-700">{item.price}</span>
                </div>
                {/* Background removed, text blends in naturally */}
                <p className="text-xs text-red-600 mt-2 font-bold">Allergens: {item.allergens}</p>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-md border border-sky-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {basicMenu.map((item, i) => (
              <div key={i} className="p-4 rounded-lg bg-slate-50 border border-slate-100 hover:bg-sky-50/50 transition-colors">
                <div className="flex justify-between items-center font-bold text-slate-800 text-lg">
                  <h3>{item.name}</h3>
                  <span className="bg-sky-100 text-sky-800 px-2 py-1 rounded text-sm">{item.price}</span>
                </div>
                {/* Background removed, text blends in naturally */}
